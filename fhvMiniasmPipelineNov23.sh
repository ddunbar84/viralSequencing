...
do
    ...
    folderName=`echo "$folder" | sed -e 's/^.*\(barcode*\/\)\///'| sed -e 's/\.fastq$//'`
# Filter reads by length and quality

    NanoFilt -l 300 -q 10 /home3/dd87a/"$project"/porechop/"$folderName"/"$folderName"Chop.fastq > /home3/dd87a/"$project"/porechop/"$folderName"/"$folderName"Filt.fastq

# Map reads to ref and move mapped reads to file
    
    minimap2 -ax map-ont "$refseq" /home3/dd87a/"$project"/porechop/"$folderName"/"$folderName"Filt.fastq | /software/samtools-v1.16.1/bin/samtools view -b -F 4 > /home3/dd87a/"$project"/porechop/"$folderName"/"$folderName"_mapped.sam

# Map reads to ref and move unmapped reads to file
    
    minimap2 -ax map-ont "$refseq" /home3/dd87a/"$project"/porechop/"$folderName"/"$folderName"Filt.fastq | /software/samtools-v1.16.1/bin/samtools view -b -f 4 > /home3/dd87a/"$project"/porechop/"$folderName"/"$folderName"_unmapped.sam

# Convert sam file of mapped reads to fastq to be used as input for de novo assembly
    
    /software/bedtools-v2.29.1/bin/bamToFastq -i /home3/dd87a/"$project"/porechop/"$folderName"/"$folderName"_mapped.sam -fq /home3/dd87a/"$project"/porechop/"$folderName"/"$folderName"_mapped.fastq 

# Filter mapped reads to minimum reads length

    /software/bbmap-v38.90/reformat.sh in=/home3/dd87a/"$project"/porechop/"$folderName"/"$folderName"_mapped.fastq  out=/home3/dd87a/"$project"/demulti_trim/"$folderName"/"$folderName"_mappedF.fastq  minlength=200

done    

##### ### GENERATE DRAFT DE NOVO GENOME ASSEMBLIES ###
