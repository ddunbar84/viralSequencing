...
for file in /home3/dd87a/"$project"/demulti_trim/*/*Chop* ; 
#for folder in /home3/dd87a/FHV_iso_26may23/demulti_trim/*/ ;

do

        folderName=`echo "$file" | sed -e 's/^.*\(barcode*\/\)\///'| sed -e 's/\.fastq$//'`

        NanoStat --fastq "$file" --outdir /home3/dd87a/"$project"/statreport -n "$fileName".txt
...
do
       ...
       minimap2 -ax map-ont "$project/$refseq" "$folderName"Chop.fastq | samtools view -bS -F 4 | samtools sort -o draftGenome.bam
