...
do
        ...
        /software/flye-v2.9.1/bin/flye --nano-hq "$sample".fastq --out-dir /home3/dd87a/"$project"/draftAssembly/"$sampleID" --meta --iterations 1
...
do
       ...
       NanoFilt -l 300 -q 10 /home3/dd87a/"$project"/demulti_trim/"$folderName"/"$folderName"Chop.fastq | minimap2 -ax map-ont "$project"/"$refseq" | samtools view -bS -F 4 | samtools sort -o /home3/dd87a/"$project"/assemblies/"$folderName"draft.bam
...
# BLAST CONTIGS AND CREATE REPORT USING DIAMOND AND KRONATOOLS

diamond blastx -d $DIAMOND_DB/ -q dopeyFoxes/draftAssembly/barcode09Chop.fasta -a diamond_output -t tempdir --top 1

diamond view -a diamond_output.daa -o diamond_output.m8

ktImportBLAST diamond_output.m8 -o diamond_output.html   # NOT WORKING - SAYS IT NEEDS SOME UPDATE SCRIPT TO BE RUN
/software/kronatools-v2.8/bin/ktImportBLAST diamond_output.m8 -o diamond_output.html


} > metagenomePipelineLog.txt
